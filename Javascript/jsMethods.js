...
const names = ["esrafil", "parsa", "atrin", "zahra", "nasim", "nazanin", "shaghayegh"];
const arrObj = [
  {name: "tom", age:25}, 
  {name: "larson", age:23}, 
  {name: "tom", age:25},
...
// unique item --> filter
let uniqueFilter = num.filter((value, index, array) => array.indexOf(value) === index);
let notUniqueFilter = num.filter((value, index, array) => array.indexOf(value) !== index);

// unique item --> reduce
let uniqueReduce = num.reduce((acc, val) => acc.includes(val) ? acc : [...acc, val], []);

// unique array of obj --> set
let uniqueObj1 = [...new Map(arrObj.map(item => [item["name"], item])).values()];
